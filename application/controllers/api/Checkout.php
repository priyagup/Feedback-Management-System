...
class checkout extends REST_Controller{
		...
		********/
        ...
        $config = array('server'            => 'https://sudip.gandhi:qKu$fQXWP4@servicemanager-cie.openmarket.com/service/v1/invokeService/3ACF90B7ED3B10FE' );
            ...
            $mydata = array("endUser"=>array("phoneNumber"=> $contact_number),
                            "variables"=> array("session"=>array("Username"=>$first_name.$last_name,"hotel"=>$hotel),"user"=>array("keyword"=>"bcns  resp","phoneNumber"=>$contact_number)));
    ...
    */
	...
	$contact_number = substr( $contact_number1, 2);
        ...
        //$mess = $returndata['message'];
        $feedback_id = $returndata['feedback_id'];
	$loyalty_id = $returndata['loyalty_id'];
        ...
        $message = array('error'=>false,'message'=>'Sucessfullyinserted','feedback_id'=>$feedback_id,'loyalty_id'=> $loyalty_id);
        $this->response($message,200);
...
log_message('info','insidwe Feedback get');
        $contact_number1 = $this->get('contact_number');
	...
	$contact_number = substr( $contact_number1, 2);
        ...
        //insert Detailed feedback
	 //insert Detailed feedback
        $feedbk_gr = explode('_', $feedback_keyword);
        $feedsub_cat_gr = explode('_',$feedback_subcat);
        //insert Detailed feedback
        
        for($i =0; $i< count($feedbk_gr) ;++$i){
        	 
        	
        	if($feedbk_gr[$i] != '' && $feedbk_gr[$i] != '_' ){
        		$data['customer_id'] = $customer_id;
        		$data['reservation_id'] = $reservation_id;
        		$data['feedback_id'] = $feedback_id;
        		$data['feedback_message'] = $feedback_message;
        		$data['feedback_keyword'] = trim($feedbk_gr[$i]);
        		$data['feedback_subcat'] = trim($feedsub_cat_gr[$i]);
        		$data['category_rating'] = $category_rating;
        		$data['feedback_time'] = $feedback_time;
